...
export default function FAQ() {
  ...
  return (
    <div className="FAQ-section" id="FAQ">
      ...
      <img src="./src/assets/faq-decor.webp" alt="FAQ Decor" className="faq-decor1"/>
      <p className="fp-heading-style">FAQ</p>
      <img src="./src/assets/faq-decor1.webp" alt="FAQ Decor" className="faq-decor" />
      <ul id="accordian">
        {questions.map((question, index) => (
          <li key={index}>
            <label htmlFor={`question-${index}`} onClick={() => handleToggle(`question-${index}`)}>
              {question} 
              <span className={rotated === `question-${index}` ? 'checked' : ''}>
            ...
            <input
              ...
              readOnly
            />
            <div className="content" style={{ display: selected === `question-${index}` ? 'block' : 'none' }}>
              <p>{answers[index]}</p>
            </div>
          </li>
        ))}
